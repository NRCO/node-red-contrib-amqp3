...
module.exports = function (RED) {
    ...
    var defaultExchangeConfig = defaultQueueOptions;
    ...
    function AmqpIn(n) {
        ...
        node.initialize = function () {
            ...
            // "3": topic exchange
            // "4": queue
            queue = node.server.connection.declareQueue(node.queue, defaultQueueOptions);
            if (node.itype !== "4") {
                exchange = node.server.connection.declareExchange(node.exchange, exchangeTypes[node.itype], defaultExchangeConfig);
                queue.bind(exchange, node.topic);
            }
            queue.activateConsumer(function (msg) {
                node.send({
    ...
    function AmqpOut(n) {
        ...
        node.initialize = function () {
            ...
            // "4": queue
            if (node.otype === "4") {
                target = node.server.connection.declareQueue(node.queue, defaultQueueOptions);
            }
            else {
                target = node.server.connection.declareExchange(node.exchange, exchangeTypes[node.otype], defaultExchangeConfig);
            }
    ...
    function AmqpServer(n) {
        ...
        node.buildUrl = function () {
            var creds = node.credentials, protocol = node.useTls ? "amqps://" : "amqp://", credentials = creds.user ? creds.user + ":" + creds.password + "@" : "", vhost = node.vhost ? "/" + node.vhost : "", params = node.keepAlive ? "heartbeat=" + node.keepAlive : "";
